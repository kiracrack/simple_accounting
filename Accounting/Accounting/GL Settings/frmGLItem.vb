...
Public Class frmGLItem
    ...
    Public Sub LoadParentItem()
        LoadToComboBoxDB("select * from tblglitem where level=" & txtLevel.Text & "-1 and groupcode='" & groupcode.Text & "' order by itemcode asc", "itemname", "itemcode", txtParent)
    End Sub
    Public Sub FilterItem()
        Try
            Em.DataSource = Nothing : dst = New DataSet
            msda = New MySqlDataAdapter("select groupcode,id,itemcode as 'Code', itemname as 'Account Title',GL,SL from tblglitem where groupcode='" & groupcode.Text & "' and parent='" & parentcode.Text & "' order by itemcode asc", conn)
            msda.Fill(dst, 0)
    ...
    Private Sub EditChapterToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles EditChapterToolStripMenuItem.Click
        ...
        While rst.Read
            ...
            txtLevel.Text = rst("level").ToString
