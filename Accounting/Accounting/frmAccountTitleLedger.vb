...
Public Class frmAccountTitleLedger
    ...
    Public Sub FilterLedger()
        Try
            ...
            Em.Columns(5).Name = "Credit"
            Em.Columns(6).Name = "Debit"
            Em.Columns(7).Name = "Balance"

            Dim cnt As Integer = 0 : Dim currbalance As Double = 0
            com.CommandText = "SELECT * FROM tblglpostingitem as a inner join tblglpostinggroup as b on a.trnno=b.trnno where a.itemcode='" & itemcode.Text & "' order by b.dateposting,a.id asc"
            ...
            While rst.Read
                If cnt = 0 Then
                    ...
                    If CDate(rst("dateposting").ToString) >= CDate(txtdatetrn.Value.ToShortDateString) Then
                        Em.Rows.Add(rst("id").ToString, rst("dateposting").ToString, rst("itemname").ToString, rst("reference").ToString, rst("details").ToString, rst("credit"), rst("debit"), currbalance)
                    End If
                Else
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        PrintDatagridview(Me.Text, "ACCOUNT LEDGER", "", Em, Me)
    End Sub
End Class
